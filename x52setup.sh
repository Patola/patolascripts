...
while [[ $X52ERR == 0 ]]; do
  ...
  x52output text 1 "XX$(LC_NUMERIC=C uptime | cut -d: -f5 | tr -d ' ') "
  x52output text 2 "XX$(ps -ef | grep 'jogos\\steam\\steamapps\\common' | grep -v grep | head -1 | sed 's/.*common\\\(.*\)\\.*/\1/' | cut -c1-15)"
done &
